...
(function ($) {
    ...
    $.fn.spartanMultiImagePicker = function (options) {
        ...
        function addRow(settings, el) {
            ...
            var template =
                `<div class="${groupClassName} spartan_item_wrapper" data-spartanindexrow="${count}" style="margin-bottom : 20px; ">` +
                ...
                if (regex.test(file_select_type) || allowedExt == "") {
                    if (
                        ...
                        reader.onload = function (e) {
                            ...
                            $(parent)
                                .find(
                                    'a[data-spartanindexremove="' + index + '"]'
                                )
                                .show();
                            $(parent)
                                .find(
                                    'img[data-spartanindeximage="' +
                                        index +
                                        '"]'
                                )
                    ...
                    if (
                        $(parent)
                            .find('img[data-spartanindeximage="' + index + '"]')
                            .is(":visible") == true
                    ) {
                        $(parent)
                            .find('img[data-spartanindexi="' + index + '"]')
        ...
        function actionDirectUpload(settings, input, parent) {
            ...
            $(parent)
                .find('[data-spartanindexloader="' + index + '"]')
                .css("display", "flex");
        ...
        function removeRow(settings, input, parent) {
            ...
            $(parent)
                .find('[data-spartanindexrow="' + index + '"]')
                .remove();
            if (
                last_index == index ||
                $(parent)
                    .find('img[data-spartanindeximage="' + last_index + '"]')
                    .is(":visible") == true
            ) {
                addRow(settings, parent);
            }
        ...
        function onDragEnter(parent) {
            ...
            if (
                ...
                $(parent)
                    .find('p[data-spartanlbldropfile="' + index + '"]')
                    .show();
                $(parent)
                    .find('img[data-spartanindexi="' + index + '"]')
                    .hide();
