...
trait PlaceNewOrder
{
    ...
    public function new_place_order(Request $request, $is_prescription = false)
    {
        ...
        try {
            ...
            //worksaar start
            if ($request->order_type == 'parcel') {
                $order->order_amount = round($order->delivery_charge + $order->dm_tips + $order->additional_charge + $order->total_tax_amount, config('round_up_to_digit'));
            }
            //worksaar end
            ...
            if ($request->order_type !== 'parcel') {
                ...
                // worksaar start
                $order->order_tax()->createMany($taxMap);
                // worksaar end
