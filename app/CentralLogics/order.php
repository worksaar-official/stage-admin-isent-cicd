...
class OrderLogic
{
    ...
    public static function cancelParcelOrder($order, $cancel_by, $request)
    {
        ...
        $return_fee_status = $parcel_cancellation_basic_setup['return_fee_status'];
        ...
        if (in_array($orderOldStatus, ['picked_up']) ) {
            ...
            if($parcel_return_time_fee_status == 1 && $return_fee_for_dm > 0){
                ...
                $parcelCancellation->return_date = now()->addDays($parcel_return_time_fee['parcel_return_time'] ?? 1);
            }
        } else{
